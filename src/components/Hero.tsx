  ...
  return (
    <section className="relative h-[600px] w-full overflow-hidden" aria-labelledby="hero-title">
      ...
      <div className="relative z-10 container mx-auto h-full flex flex-col items-start justify-center text-white px-4 md:px-8">
        <h2 id="hero-title" className="text-5xl md:text-6xl lg:text-7xl font-bold mb-4 max-w-3xl leading-tight">
          {title}
        </h2>
        <p className="text-xl md:text-2xl mb-8 font-medium">{subtitle}</p>
        <div className="flex items-center gap-6 mb-8" role="navigation" aria-label="Social Media Links">
        ...
        <Button
          size="lg"
          variant="secondary"
          className="font-semibold px-8 py-6 text-lg shadow-lg hover:shadow-xl transition-all focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-transparent"
          asChild
        >
          <a href={ctaHref || "#"}>
            {ctaText}
